...
The customer can now bring the purchase card to the till without starting on their phone. Start at `/verify/purchase`, also linked from `/verify` as **Sell a Zik Pass at the till**.

1. Select the terminal's store and start a purchase sale.
...
- Generic enrollment cannot claim a purchase sale by guessing its session ID.
