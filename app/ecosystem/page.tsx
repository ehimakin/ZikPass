    ...
    <CustomerShell active="about" title="The Zik ecosystem">
      <div className="space-y-6 pb-6">
        ...
        <Card as="section" className="p-5">
          ...
          <p className="mt-3 text-sm leading-relaxed text-[var(--zk-text-soft)]" data-local-edit={process.env.NODE_ENV === "development" ? "ve-2b27d2feb142-8" : undefined}>ZikVault is planned as a local-first pool of proof-backed credential objects. Credentials and underlying identity documents are designed to live on your device rather than in a central Zik identity-document database.</p>
          <p className="mt-3 text-sm leading-relaxed text-[var(--zk-text-soft)]" data-local-edit={process.env.NODE_ENV === "development" ? "ve-2b27d2feb142-9" : undefined}>Future evidence could include a passport, driving licence, utility bill, designated verified photograph, address evidence, or student and professional credentials. You would choose which verified claims to combine for a request.</p>
          <p className="mt-3 text-sm leading-relaxed text-[var(--zk-text-soft)]" data-local-edit={process.env.NODE_ENV === "development" ? "ve-2b27d2feb142-10" : undefined}>When enough verified information is available, an in-app offer could let you assemble Zik ID: a preset combining Vault claims such as verified name, photograph, age and identity-document status. It would not create an independent profile or duplicate identity database.</p>
        </Card>
        <Card as="section" className="p-5">
        ...
        <p className="text-xs leading-relaxed text-[var(--zk-text-soft)]" data-local-edit={process.env.NODE_ENV === "development" ? "ve-2b27d2feb142-13" : undefined}>Local-first does not mean Zik stores no data. Operational services may still hold public keys, status and revocation data, issuer metadata, fraud signals and audit events. The future Vault needs a separate security and data architecture decision before implementation. Planned prices cannot be charged here, and Zik ID is not currently accepted as physical identification.</p>
      </div>
    </CustomerShell>
  );
}
