...
const summary = (session: PhysicalStoreSessionRecord) => ({
  ...
  expiresAt: session.expires_at, amountMinor: session.counter_sale!.amount_minor,
  currency: session.counter_sale!.currency, paid: Boolean(session.counter_sale!.paid_at),
  verified: session.clerk_verification.status === "verified", status: session.status,
  claimed: Boolean(session.enrollment_id)
});

export async function startCounterSale(storeId: string, verifierToken?: string) {
  ...
  const session: PhysicalStoreSessionRecord = {
    ...
    counter_sale: { token_hash: hash(token), amount_minor: runtimeConfig.passIssuancePriceMinor, currency: runtimeConfig.deviceExtensionCurrency }
  };
  return runCounterSaleTransaction((data) => {
    data.physical_sessions.push(session);
    return { ...summary(session), token };
  });
}

export async function updateCounterSale(input: {
  ...
  return runCounterSaleTransaction((data) => {
    ...
    if (!session?.counter_sale || session.store_id !== verifier.retailer_id) throw new Error("Sale not found for this store.");
    ...
    if (input.action === "renew") {
      if (!session.counter_sale.paid_at || session.clerk_verification.status !== "verified") throw new Error("Only a verified, paid sale can receive a replacement QR.");
      const token = randomBytes(32).toString("base64url");
      session.counter_sale.token_hash = hash(token);
    ...
    if (input.action === "confirm_id") {
      ...
      if (session.counter_sale.paid_at) throw new Error("Payment is already recorded. Resolve this sale at the till.");
      ...
      if (!session.counter_sale.paid_at) {
        session.counter_sale.paid_at = now;
        session.counter_sale.paid_by = verifier.verifier_id;
        session.counter_sale.payment_method = input.method;
...
export async function claimCounterSale(token: string, holderPublicKey: JsonWebKey): Promise<EnrollmentRecord> {
  ...
  return runCounterSaleTransaction((data) => {
    const session = data.physical_sessions.find((s) => s.counter_sale?.token_hash === hash(token));
    if (!session?.counter_sale) throw new Error("Activation link not found. Ask the clerk for your QR.");
    const sale = session.counter_sale;
    ...
    const payment: PaymentRecord = {
      payment_id: randomId("pay"), idempotency_key: `counter:${session.session_id}`, enrollment_id: record.id,
