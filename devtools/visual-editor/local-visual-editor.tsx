...
export function LocalVisualEditor(){
  ...
  const start=useCallback(async()=>{
    setBusy(true);
    try{
      // Resume if Next needs a full reload after mapping a server component.
      sessionStorage.setItem('local-visual-editor:resume','true');
      const initial=await refresh();
      const response=await fetch('/api/local-editor',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({token:initial.token})});
      const result=await response.json();if(!response.ok)throw Error(result.error??'Unable to map editable text.');
      await refresh();setEnabled(true);setMessage('Click outlined text. New pages are mapped automatically; dynamic application values are not editable.');
  ...
  useEffect(()=>{if(!enabled)return;document.documentElement.classList.add(styles.editing);
    function choose(event:MouseEvent){const target=event.target;if(!(target instanceof Element)||panel.current?.contains(target))return;
      ...
      if(!element||!item){setMessage('This text is dynamic or outside the configured folders. Choose outlined text, or refresh editable text after a source change.');return;}
      restore();const nodes=textNodes(element);if(nodes.length!==item.texts.length||nodes.some((node,i)=>(node.textContent??'').replace(/\s+/g,' ').trim()!==item.texts[i].replace(/\s+/g,' ').trim())){setMessage('Rendered text differs from its source. Reload and try again.');return;}
  ...
  return <aside ref={panel} className={styles.editor} aria-label="Local visual editor">
    ...
    {!enabled?<button onClick={()=>void start()} disabled={busy}>{busy?"Mapping editable text…":"Edit page"}</button>:<>
      <button onClick={()=>{restore();setEntry(undefined);void start();}} disabled={busy}>Refresh editable text</button>
