...
function PurchaseSaleForm({ storeId }: { storeId: string }) {
  ...
  return <div className="space-y-4">
    <p className="text-sm" data-local-edit={process.env.NODE_ENV === "development" ? "ve-d3d24a6cee7c-1" : undefined}>Purchase card → check ID → take payment → hand over activation QR.</p>
    <p className="text-xs text-[var(--zk-text-soft)]" data-local-edit={process.env.NODE_ENV === "development" ? "ve-d3d24a6cee7c-2" : undefined}>Demo terminal. Payment records represent money taken at the till; this screen does not charge a card.</p>
    {error && <Alert tone="critical">{error}</Alert>}
    {!sale ? <Card className="space-y-4 p-5"><p data-local-edit={process.env.NODE_ENV === "development" ? "ve-d3d24a6cee7c-3" : undefined}>Customer brought a Zik purchase card to the till? Start their sale here. They do not need their phone yet.</p><Button loading={busy} onClick={() => void action("start")}>Start sale</Button><ButtonLink href="/verify" variant="secondary">Customer already has a verification code</ButtonLink></Card>
    : sale.status === "rejected" ? <Card className="space-y-4 p-5"><p data-local-edit={process.env.NODE_ENV === "development" ? "ve-d3d24a6cee7c-4" : undefined}>ID not verified. Do not take payment or issue a pass.</p><Button onClick={reset}>Next customer</Button></Card>
    ...
    : !sale.verified ? <Card className="space-y-4 p-5"><h2 className="font-bold" data-local-edit={process.env.NODE_ENV === "development" ? "ve-d3d24a6cee7c-5" : undefined}>1. Check photo ID</h2><p data-local-edit={process.env.NODE_ENV === "development" ? "ve-d3d24a6cee7c-6" : undefined}>Confirm the customer is 18 or over. Inspect their ID in person; do not photograph or upload it.</p><Button loading={busy} onClick={() => void action("confirm_id")}>ID checked — confirm 18+</Button><Button variant="danger" disabled={busy} onClick={() => void action("reject")}>Cannot verify</Button></Card>
    : !sale.paid ? <Card className="space-y-4 p-5"><h2 className="font-bold" data-local-edit={process.env.NODE_ENV === "development" ? "ve-d3d24a6cee7c-7" : undefined}>2. Take payment</h2><p>ID confirmed. Collect {new Intl.NumberFormat("en-GB", { style: "currency", currency: sale.currency }).format(sale.amountMinor / 100)} at the till.</p><label className="block">Payment method<select className="mt-2 block w-full rounded-xl border p-3" value={method} onChange={(event) => setMethod(event.target.value)}><option value="cash">Cash</option><option value="retail_till">Card at the till</option></select></label><Button loading={busy} onClick={() => void action("confirm_payment")}>Payment received — show QR</Button></Card>
    : <Card className="space-y-4 p-5"><h2 className="font-bold" data-local-edit={process.env.NODE_ENV === "development" ? "ve-d3d24a6cee7c-8" : undefined}>3. Customer scans to receive their pass</h2><p data-local-edit={process.env.NODE_ENV === "development" ? "ve-d3d24a6cee7c-9" : undefined}>ID and payment confirmed. Ask this customer to scan the QR on their own phone, then choose “Save my Zik Pass”.</p>{qr && <Image src={qr} alt="Scan to activate your paid Zik Pass" width={320} height={320} className="mx-auto rounded-none" unoptimized />}<p className="text-center text-sm">Activation window: {Math.max(0, Math.ceil((Date.parse(sale.expiresAt) - now) / 60000))} minutes remaining</p><a className="block text-center underline" href={url} target="_blank" rel="noreferrer" data-local-edit={process.env.NODE_ENV === "development" ? "ve-d3d24a6cee7c-10" : undefined}>Open activation link (same-device demo)</a><Button loading={busy} onClick={() => void action("status")}>Check customer progress</Button><p className="text-xs" data-local-edit={process.env.NODE_ENV === "development" ? "ve-d3d24a6cee7c-11" : undefined}>This private QR is for this customer only. Refreshing this terminal keeps the current sale.</p></Card>}
  </div>;
}
