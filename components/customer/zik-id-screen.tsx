...
export function ZikIdScreen() {
  ...
  if (vaultExists === false) return <Alert tone="caution" title="Create your Vault first" action={<ButtonLink href="/vault-legacy">Open Zik Vault</ButtonLink>}>Your designated selfie and optional identity fields stay encrypted in your device Vault.</Alert>;

  return (
    <div className="space-y-5">
      <div><div className="flex items-center gap-2"><h1 className="text-2xl font-extrabold" data-local-edit={process.env.NODE_ENV === "development" ? "ve-be4a7bf9d1bb-1" : undefined}>Present Zik ID</h1><StatusBadge>Peer-to-peer</StatusBadge></div><p className="mt-2 text-sm text-[var(--zk-text-soft)]" data-local-edit={process.env.NODE_ENV === "development" ? "ve-be4a7bf9d1bb-2" : undefined}>Build a one-time bar ID from your device Vault. Zik carries connection details only—the selfie and selected fields do not pass through Zik servers.</p></div>

      {!profile ? <Card className="space-y-4 p-5"><label className="block text-sm font-semibold">Vault passphrase<input type="password" minLength={12} maxLength={1024} value={passphrase} onChange={(event)=>setPassphrase(event.target.value)} className="mt-1 w-full rounded-lg border p-3" autoComplete="off"/></label><Button onClick={()=>void unlockVault()} loading={busy} disabled={passphrase.length<12}>Unlock Vault</Button></Card> : !profile.selfie ? <Alert tone="caution" title="Choose a designated selfie" action={<ButtonLink href="/vault-legacy">Add selfie in Vault</ButtonLink>}>The photo must be selected and encrypted on this device before Zik ID can be used.</Alert> : (
        <>
          <Card className="space-y-4 p-5"><div className="flex items-center gap-4">{/* eslint-disable-next-line @next/next/no-img-element */}<img src={profile.selfie.data_url} alt="Your designated Zik ID selfie" className="h-24 w-24 rounded-none object-cover"/><div><p className="font-bold" data-local-edit={process.env.NODE_ENV === "development" ? "ve-be4a7bf9d1bb-3" : undefined}>Designated selfie</p><p className="mt-1 text-xs text-[var(--zk-text-soft)]" data-local-edit={process.env.NODE_ENV === "development" ? "ve-be4a7bf9d1bb-4" : undefined}>Encrypted in this device Vault</p></div></div><fieldset className="space-y-3"><legend className="font-bold">Share for this check</legend><CheckedRow label="Over 18" note="Zik verified · required" checked disabled/><CheckedRow label="Legal name" note="Self-entered" checked={includeName} onChange={setIncludeName}/><CheckedRow label="Delivery address" note="Self-entered" checked={includeAddress} onChange={setIncludeAddress}/>{profile.email?<CheckedRow label="Email" note="Self-entered" checked={includeEmail} onChange={setIncludeEmail}/>:null}</fieldset></Card>
          {!signal ? <Button size="lg" loading={busy} onClick={()=>void prepareId()}>Authenticate and create one-time ID</Button> : <Card className="space-y-3 p-5 text-center"><p className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--zk-text-soft)]" data-local-edit={process.env.NODE_ENV === "development" ? "ve-be4a7bf9d1bb-5" : undefined}>Bouncer scans this</p>{qr?/* eslint-disable-next-line @next/next/no-img-element */<img src={qr} alt="One-time Zik ID QR code" className="mx-auto w-full max-w-[280px] rounded-none"/>:null}<p className="font-mono text-2xl font-bold tracking-[0.18em]">{signal.code}</p><p className="text-xs text-[var(--zk-text-soft)]">Expires {new Date(signal.expiresAt).toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"})}</p><a href={signal.verifyUrl} className="text-sm font-semibold underline" data-local-edit={process.env.NODE_ENV === "development" ? "ve-be4a7bf9d1bb-6" : undefined}>Open verifier on this device</a></Card>}
        </>
      )}
      <p role="status" aria-live="polite" className="rounded-none bg-[var(--zk-sunken)] p-3 text-sm">{status}</p>
      <p className="text-xs leading-relaxed text-[var(--zk-text-faint)]" data-local-edit={process.env.NODE_ENV === "development" ? "ve-be4a7bf9d1bb-7" : undefined}>WebRTC encrypts the direct device-to-device transfer. Connection signaling can expose network metadata such as IP addresses to the paired devices. No browser API can invoke AirDrop, and Web NFC is not consistently available on iPhone.</p>
    </div>
  );
}
