...
export function RetailDemo() {
  ...
  return <div className="mx-auto max-w-[460px] space-y-4 px-4 py-6">
    <Card className="space-y-4 p-5"><p className="text-xs uppercase tracking-widest">Fictional retail demo</p><h1 className="text-2xl font-bold">Harbour &amp; Pine</h1><p>Evening hamper · £24.00</p><p className="text-sm">Delivery checkout</p><p role="status" aria-live="polite">{message}</p>
      {!request&&<button ref={trigger} disabled={busy} onClick={()=>void start()} className="min-h-12 rounded-full bg-ink px-6 py-3 font-semibold text-white disabled:opacity-50">Fill with Zik</button>}
      {filled&&<div className="space-y-3"><StatusBadge tone="positive">Age 18+ · Zik verified</StatusBadge>{Object.entries(filled).map(([f,value])=><label key={f} className="block text-sm">{fieldLabels[f as ProfileField]} <StatusBadge>Self-entered</StatusBadge><textarea aria-label={`Checkout ${fieldLabels[f as ProfileField]}`} value={value} readOnly className="mt-1 w-full rounded-lg border p-3"/></label>)}<p className="text-sm">No purchase is submitted in this demo.</p></div>}
    </Card>
    {request&&<section aria-label="Disclosure consent" className="space-y-4">
      <Card className="space-y-3 p-5"><h2 tabIndex={-1} ref={heading} className="text-xl font-bold">Share with {request.display_name}?</h2><p>{request.purpose}</p><p className="text-sm">Request expires in two minutes. Protocol v1.</p><h3 className="font-bold">Required</h3><p>Age over 18 <StatusBadge tone="positive">Zik verified</StatusBadge></p><p>Legal name and delivery address <StatusBadge>Self-entered</StatusBadge></p><h3 className="font-bold">Optional · off by default</h3><label className="flex gap-2"><input type="checkbox" checked={email} disabled={!profile?.email||busy} onChange={e=>setEmail(e.target.checked)}/>Share email · Self-entered</label><p className="text-sm">Not shared: date of birth, photo, ID number, raw credential or holder key. Name and address are not Zik-verified.</p><p className="text-sm">Approved profile fields are encrypted for this merchant. This co-hosted demo merchant can read them after approval.</p></Card>
      <VaultScreen onUnlocked={unlocked}/>
      {profile&&<Card className="space-y-2 p-5"><h3 className="font-bold">You will share</h3>{(['legal_name','delivery_address',...(email?['email']:[])] as ProfileField[]).map(f=><p key={f} className="break-words">{fieldLabels[f]}: {profile[f]?.value} <StatusBadge>Self-entered</StatusBadge></p>)}</Card>}
    ...
    <ButtonLink href="/vault" variant="ghost">My Vault</ButtonLink><ButtonLink href="/affiliate-demo" variant="ghost">Try age-only</ButtonLink>
  </div>;
}
