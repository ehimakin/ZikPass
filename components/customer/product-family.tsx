  ...
  return (
    <ol className="space-y-3" aria-label="Zik product progression">
      {getProductCatalogue(price).map((product, index) => (
        <li key={product.name}>
          {index > 0 ? <p aria-hidden="true" className="mb-3 pl-6 text-xl text-[var(--zk-text-faint)]">↓</p> : null}
          <Card as="article" className={`p-5 ${product.available ? "border-[var(--zk-accent)] border-l-4" : ""}`}>
            ...
            {product.available ? <ButtonLink href={product.destination} className="mt-4">Get Zik Pass · {price}</ButtonLink> : <p className="mt-2 text-xs text-[var(--zk-text-soft)]">Planned concept · Not available in this prototype</p>}
          </Card>
        </li>
      ))}
    </ol>
