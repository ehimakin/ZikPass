...
export function ZikIdVerifier({ sessionId, code, issuerPublicKey }: { sessionId: string; code: string; issuerPublicKey: JsonWebKey }) {
  ...
  return (
    <main className="mx-auto min-h-dvh max-w-md bg-[var(--zk-canvas)] px-4 py-6 text-[var(--zk-text)]">
      ...
      {payload ? <section className="overflow-hidden rounded-[28px] border border-[#d3bb53] bg-[#fffdf5] shadow-xl"><div className="bg-[#28623c] px-5 py-4 text-white"><div className="flex items-center justify-between"><strong className="text-xl" data-local-edit={process.env.NODE_ENV === "development" ? "ve-e64ed15cdcfc-2" : undefined}>18+ verified</strong><span className="rounded-full bg-white/15 px-3 py-1 text-sm">{remaining}s</span></div><p className="mt-1 text-sm text-white/75" data-local-edit={process.env.NODE_ENV === "development" ? "ve-e64ed15cdcfc-3" : undefined}>Signed Zik Pass verified on this device</p></div><div className="space-y-5 p-5">{/* eslint-disable-next-line @next/next/no-img-element */}<img src={payload.selfie} alt="Temporary designated photo of the Zik ID holder" className="aspect-square w-full rounded-3xl object-cover"/><div className="rounded-xl bg-[#fff3bf] p-3 text-center font-bold text-[#5f4d00]">Compare this photo with the person in front of you</div><dl className="space-y-3">{payload.disclosed.legalName?<Field label="Legal name" value={payload.disclosed.legalName}/>:null}{payload.disclosed.deliveryAddress?<Field label="Delivery address" value={payload.disclosed.deliveryAddress}/>:null}{payload.disclosed.email?<Field label="Email" value={payload.disclosed.email}/>:null}</dl><Button variant="secondary" size="lg" onClick={()=>{setPayload(undefined);setRemaining(0);setStatus("Photo cleared by verifier.");}}>Clear now</Button></div></section> : <section className="grid min-h-[60vh] place-items-center rounded-[28px] border border-[var(--zk-line)] bg-[var(--zk-card)] p-8 text-center"><div><div className="mx-auto mb-5 h-12 w-12 animate-pulse rounded-full bg-[var(--zk-positive-bg)]"/><h1 className="text-2xl font-extrabold" data-local-edit={process.env.NODE_ENV === "development" ? "ve-e64ed15cdcfc-4" : undefined}>One-time Zik ID</h1><p role="status" className="mt-3 text-sm leading-relaxed text-[var(--zk-text-soft)]">{status}</p><p className="mt-5 font-mono text-lg font-bold tracking-[0.18em]">{code}</p></div></section>}
